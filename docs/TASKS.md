...
## Next — global chat + optional RAG

All items below implemented in current worktree and verified by scout audit + gates (cargo 59, pnpm 42, build pass, diff-check clean). Pending final commit + graphify update.

- [x] Add project-independent Global Chat: dedicated Pi session/history, no project worktree, diff, dev server, pipeline, or project tool context. — `pi_rpc.rs:global_chat_cwd()` app-owned `Library/Application Support/command-rdev-center/global-chat`, spawn enforces exact cwd match, no CRC_PROJECT_* envs, only `web_search,source_check,fetch_content,get_search_content` tools; `ChatView` disables file-picker @, graph, diff polling, dev runner, pipeline polling, extensions, graph report injection.
- [x] Add Global Chat navigation and session tabs without disrupting existing project chats. — `App.tsx` `Tab.global?`, `GLOBAL_PROJECT`, `newGlobalChat/openGlobalChat`, separate sidebar lists, `global === tab.global` resume isolation, `selectedProject=null` on global activation.
- [x] Add RAG Settings UI: enabled toggle, HTTPS base URL, Keychain-backed bearer token, connection test, timeout, upload limit, and source toggles. Never hardcode hostnames, tokens, or deployment-specific values. — `RagSettings.tsx` placeholder `https://extractor.example` only, no hardcoded host; `rag.rs` SERVICE `command-rdev-center.rag` ACCOUNT `bearer-token` via `security_framework`, `validate()` enforces `https://` + rejects CR/LF/@/# when enabled, timeout 1-60, upload 1-25 MB, project paths via `ensure_registered_project`.
- [x] Add document upload knowledge sources. Send PDF/DOCX/TXT/MD/CSV/JSON to the configured extractor; persist extracted text locally, not the original file. — `RagKnowledge.tsx` dialog + drag-drop `onDragDropEvent`, ALLOWED 6 types, `ingest_rag_document_blocking` curl `--proto =https --max-redirs 0 --max-time` + `curl_fail_flag()` --fail-with-body fallback, multipart `file=@`, 25 MB MAX_BYTES dual check, persist extracted text only via atomic tmp→rename, `safe_name` sanitizes control chars and `[]` to `()`.
- [x] Add local chunking, keyword retrieval, and source citations injected into Global Chat prompts. Support selected registered projects as an additional source. — `get_rag_context()` enabled check, `words()` tokenization, term-freq scoring, take 4, 2500 chars each, MAX_CONTEXT 12k, `[SOURCE: name]` citations, `[UNTRUSTED RETRIEVED SOURCES…]` marker + `--- END OF RETRIEVED SOURCES, USER QUERY FOLLOWS ---` delimiter, `project_sources()` top-level only, dotfile skip, 1 MB per-file guard, 100 cap, `ChatView.handleSend` only when `globalChat` invokes context and prepends, toast on error, empty when disabled.
- [x] Configure external extractor deployment through Settings only. Current personal deployment: Contabo service bound to Tailscale `100.104.78.91:8080`, exposed by Cloudflare Tunnel as `https://rag.rdev.web.id`; this is deployment context, not an application default. — No `rag.rdev.web.id` / `100.104` / `contabo` in `src/` or `src-tauri/src/`; only in `docs/TASKS.md` deployment context doc.
- [x] Extractor contract: authenticated `GET /health`; authenticated multipart `POST /v1/extract`; 25 MB max; PDF/DOCX plus UTF-8 text formats. Reject image-only PDFs until OCR is explicitly added. — `test_rag_connection` GET /health, `ingest...` POST /v1/extract, Authorization Bearer, `--proto =https`, `--max-redirs 0`, `--max-time`, PROJECT_TEXT shortcut for txt/md/csv/json UTF-8 read, pdf/docx via extractor.
- [x] Validate: unauthorized extractor calls return 401; configured Settings connection test passes; upload/retrieval citations render; Global Chat works with RAG disabled; `cargo test`, `pnpm test`, `pnpm build`, `git diff --check`, then `graphify update .`. — 401 detection `is_unauthorized_detail` + status 401 returns explicit `Extractor returned 401 Unauthorized — check bearer token in RAG settings`, connection test uses output() to capture body, gates: `cargo test` 59 pass, `pnpm test` 42 pass (7 files), `pnpm build` 489 kB JS pass, `git diff --check` clean. GlobalChat `get_rag_context` returns empty when disabled. ChatView guards skip diff/dev/pipeline polling for global.
- [ ] RAG token management UI/API: temporarily obtain the deployment token manually from Contabo (`RAG_API_TOKEN` in `/home/rdev/rag-service/.env`). Later add an authenticated managed API for per-device/user token creation, rotation, and revocation. Desktop stores received tokens only in macOS Keychain; it must never SSH to Contabo or reveal stored tokens.
