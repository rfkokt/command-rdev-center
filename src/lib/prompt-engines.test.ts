...
const engine = (enabled: boolean, mode: "auto" | "review"): PromptEngine => ({
  version: 1, id: "x", name: "X", icon: "X", description: "", system_prompt: "FORMAT AS JSON", starter_message: "Start", model: "", thinking: "", research: { enabled, mode, instructions: "Prefer primary sources." }, created_at: 1, updated_at: 1,
});

describe("runtimePrompt", () => {
  it("does not alter engines without research", () => expect(runtimePrompt(engine(false, "auto"))).toBe("FORMAT AS JSON"));
