...
export type PromptEngineInput = Omit<
  PromptEngine,
  "version" | "id" | "created_at" | "updated_at"
> & { id?: string };

export const listPromptEngines = () =>
  invoke<PromptEngine[]>("list_prompt_engines");
export const savePromptEngine = (input: PromptEngineInput) =>
  invoke<PromptEngine>("save_prompt_engine", { input });
export const deletePromptEngine = (id: string) =>
  invoke<void>("delete_prompt_engine", { id });

export function runtimePrompt(engine: PromptEngine) {
  if (!engine.research.enabled) return engine.system_prompt;
  const gate =
    engine.research.mode === "review"
