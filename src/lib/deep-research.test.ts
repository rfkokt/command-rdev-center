...
import { canResumeResearch, extractToc, slugifyHeading, sortResearchRuns, type ResearchRun } from "./deep-research";
...
describe("deep research view model", () => {
  ...
  it("slugifies headings and deduplicates via extractToc", () => {
    ...
    expect(toc.map((e) => ({ text: e.text, id: e.id, level: e.level }))).toEqual([
