  ...
  invoke.mockResolvedValueOnce([
    {
      project: "sheet",
      read_only: true,
      tasks: [{ no: 1, deskripsi: "Sheet task", status: "Review" }],
    },
    {
      project: "broken",
      read_only: true,
    ...
    {
      project: "local",
      tasks: [{ no: 2, deskripsi: "Local task", status: "Backlog" }],
    },
  ]);
...
test("opens the full task text in an accessible detail dialog", async () => {
  ...
  invoke.mockResolvedValueOnce([
    {
      project: "alpha",
      read_only: true,
      tasks: [{ no: 1, deskripsi: detail, pic: "Rifky", status: "To Do" }],
    },
  ]);
  ...
  expect(screen.getByRole("dialog", { name: "Task detail" })).toHaveTextContent(
    "First requirement",
  );
  expect(screen.getByRole("dialog", { name: "Task detail" })).toHaveTextContent(
    "curl https://example.com",
...
test("scopes tasks to the selected project", async () => {
  invoke.mockResolvedValueOnce([
    {
      project: "alpha",
      tasks: [{ no: 1, deskripsi: "Alpha task", status: "Backlog" }],
    },
    {
      project: "beta",
      tasks: [{ no: 2, deskripsi: "Beta task", status: "Backlog" }],
    },
  ]);
...
test("renders spreadsheet statuses as separate columns", async () => {
  ...
  expect(
    screen.getByText("Pending", { selector: ".kanban-column > header strong" }),
  ).toBeInTheDocument();
  expect(
    screen.getByText("To Do", { selector: ".kanban-column > header strong" }),
  ).toBeInTheDocument();
  expect(
    screen.getByText("Testing", { selector: ".kanban-column > header strong" }),
  ).toBeInTheDocument();
});
    ...
    .mockResolvedValueOnce([
      {
        project: "alpha",
        read_only: true,
        tasks: [{ no: 1, deskripsi: "Sprint 0 task", status: "Backlog" }],
      },
    ])
    .mockResolvedValueOnce([
      {
        project: "alpha",
        read_only: true,
        tasks: [{ no: 2, deskripsi: "Sprint 1 task", status: "Backlog" }],
      },
    ]);
...
test("groups local statuses case-insensitively, filters, and writes canonical status", async () => {
    ...
    .mockResolvedValueOnce([
      {
        project: "alpha",
        tasks: [
          { no: 1, deskripsi: "Ship alpha", pic: "Rifki", status: "backlog" },
        ],
      },
      {
        project: "beta",
        tasks: [
          { no: 2, deskripsi: "Review beta", pic: "Agent", status: "REVIEW" },
        ],
      },
    ])
  ...
  await waitFor(() =>
    expect(invoke).toHaveBeenLastCalledWith("update_kanban_task_status", {
      project: "alpha",
      taskNo: 1,
      status: "Done",
    }),
