...
const live = {
  current: {
    ...
    status: "running",
    stages: [{ name: "Deploy", status: "running" }],
  },
  runs: [
    {
      run_id: "old",
      ...
      status: "pass",
      stages: [{ name: "Test", status: "pass", ms: 1000 }],
    },
    {
      run_id: "new",
      project: "demo-new",
      ...
      status: "pass",
      stages: [{ name: "Test", status: "pass", ms: 3000 }],
    },
  ],
};
...
test("renders live and dynamic stages with averages and newest runs first", async () => {
  ...
  expect(
    within(liveTable).getByRole("columnheader", { name: /Deploy/ }),
  ).toBeInTheDocument();
  expect(within(liveTable).getByText("running")).toBeInTheDocument();

  const historical = screen.getByRole("table", {
    name: /KAI pipeline · alternate/,
  });
...
test("renders unix-second timestamps without Invalid Date", async () => {
  invoke.mockResolvedValue({
    current: null,
    runs: [{ ...live.runs[0], date: "1785480149" }],
  });
  render(<PipelineView />);
...
test("submits only a configured confirm option", async () => {
  ...
  await waitFor(() =>
    expect(invoke).toHaveBeenCalledWith("provide_pipeline_input", {
...
test("refreshes immediately after starting and hides duplicate run action", async () => {
  invoke
    .mockResolvedValueOnce({ current: null, runs: [] })
  ...
  await waitFor(() =>
    expect(invoke).toHaveBeenCalledWith("start_pipeline", {
      projectPath: "/projects/demo",
      executionCwd: null,
