...
export default function PipelineSettings({ projectPath, projectName, onToast }: { projectPath: string; projectName?: string; onToast: (message: string) => void }) {
  ...
  async function startConsultant() {
    ...
    try {
      ...
      await invoke("spawn_pi_rpc", { sessionId, cwd: projectPath, model: null, provider: null, thinking: "medium", noSession: true, sessionFile: null, graphReportPath: null, tools: [] });
      if (generation !== consultantGeneration.current) { resetConsultant(); return; }
      const system = `You are a pipeline configuration consultant for ${projectName ?? projectPath}. Ask concise questions until you understand desired validation, build, git, deploy, failure and confirmation steps. Never run tools or commands. When ready, include exactly one fenced JSON object matching this TypeScript shape: {preset:\"Personal\"|\"KAI\"|\"MBI\"|\"Custom\",steps:[{id:string,name:string,command:string,enabled:boolean,failure_policy:\"ai_fix\"|\"ask_user\"|\"stop\"|\"continue\",max_attempts:number}]}. Commands may be proposed but are never executed or saved automatically. Current config: ${JSON.stringify(config)}. Begin with the most important missing question.`;
