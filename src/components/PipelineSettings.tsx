...
export type PipelineStep = {
  id: string;
  name: string;
  mode?: "shell" | "ai_commit" | "confirm";
  command: string;
  enabled: boolean;
  failure_policy: "ai_fix" | "ask_user" | "stop" | "continue";
  max_attempts: number;
  prompt?: string;
  options?: string[];
};
export type PipelineConfig = {
  preset: "Personal" | "KAI" | "MBI" | "Custom";
  steps: PipelineStep[];
};

const PRESETS: PipelineConfig["preset"][] = [
  "Personal",
  "KAI",
  "MBI",
  "Custom",
...
const POLICIES: Array<{
  value: PipelineStep["failure_policy"];
  label: string;
  detail: string;
}> = [
  {
    value: "ai_fix",
    label: "AI fix",
    detail: "Pause, let the active agent diagnose, then retry.",
  },
  {
    value: "ask_user",
    label: "Ask user",
    detail: "Pause for approval. Best for push, merge, deploy, and release.",
  },
  { value: "stop", label: "Stop", detail: "End the pipeline immediately." },
  {
    value: "continue",
    label: "Continue",
    detail: "Record the failure and run the next step.",
  },
];
...
function consultantDraft(text: string): PipelineConfig | null {
  ...
  try {
    ...
    if (
      ...
      typeof value !== "object" ||
      Array.isArray(value) ||
      !exactKeys(value as Record<string, unknown>, CONFIG_KEYS)
    )
      return null;
    const candidate = value as Record<string, unknown>;
    if (
      typeof candidate.preset !== "string" ||
      !["Personal", "KAI", "MBI", "Custom"].includes(candidate.preset) ||
      !Array.isArray(candidate.steps) ||
      candidate.steps.length > 50
    )
      return null;
    if (
      candidate.steps.some((raw) => {
        if (!raw || typeof raw !== "object" || Array.isArray(raw)) return true;
        const keys = Object.keys(raw as Record<string, unknown>);
        if (
          REQUIRED_STEP_KEYS.some((key) => !keys.includes(key)) ||
          keys.some(
            (key) =>
              !REQUIRED_STEP_KEYS.includes(key) &&
              !OPTIONAL_STEP_KEYS.includes(key),
          )
        )
          return true;
        ...
        return (
          (step.mode !== undefined &&
            !["shell", "ai_commit", "confirm"].includes(String(step.mode))) ||
          (step.prompt !== undefined && typeof step.prompt !== "string") ||
          (step.options !== undefined &&
            (!Array.isArray(step.options) ||
              step.options.some((option) => typeof option !== "string"))) ||
          ...
          !step.command.trim() ||
          step.command.length > 2_000 ||
          typeof step.enabled !== "boolean" ||
          typeof step.failure_policy !== "string" ||
          !["ai_fix", "ask_user", "stop", "continue"].includes(
            step.failure_policy,
          ) ||
...
export default function PipelineSettings({
  ...
  const consultantRef = useModalFocus<HTMLElement>(
    closeConsultant,
    consultOpen,
  );
  ...
  useEffect(() => {
    ...
    setConfig(null);
    void invoke<PipelineConfig>("get_pipeline_config", {
      projectPath,
      preset: null,
    })
      .then((next) => {
  ...
  if (!config)
    return (
      <div className="settings-loading">{error || "LOADING PIPELINE…"}</div>
    );

  const update = (index: number, patch: Partial<PipelineStep>) =>
    setConfig({
      ...config,
      preset: "Custom",
  ...
  const invalid = config.steps.some(
    (step) =>
      !step.name.trim() ||
      ((step.mode ?? "shell") !== "ai_commit" && !step.command.trim()) ||
      step.max_attempts < 1 ||
      step.max_attempts > 10,
  );
  ...
  async function selectPreset(preset: PipelineConfig["preset"]) {
    if (preset === "Custom")
      return setConfig((current) => current && { ...current, preset });
    if (
      dirty &&
      !(await confirm({
        title: "Apply preset",
        message: `Replace unsaved steps with the ${preset} preset?`,
    ...
    const request = ++requestRef.current;
    try {
      const next = await invoke<PipelineConfig>("get_pipeline_config", {
        projectPath,
        preset,
      });
  ...
  async function startConsultant() {
    ...
    setConsultError("");
    try {
      const eventUnlisten = await listen<{ session_id: string; raw: string }>(
        "pi-rpc-event",
        (event) => {
          if (event.payload.session_id !== sessionId) return;
          try {
            const rpc = JSON.parse(event.payload.raw) as Record<
              string,
              unknown
            >;
            if (generation !== consultantGeneration.current) return;
            if (rpc.type === "message_update") {
              const update = rpc.assistantMessageEvent as
                | Record<string, unknown>
                | undefined;
              if (update?.type === "text_delta")
                consultantText.current = (
                  consultantText.current + String(update.delta ?? "")
                ).slice(-100_000);
            }
            ...
            /* ignore unrelated RPC frames */
          }
        },
      );
      const errorUnlisten = await listen<{ session_id: string; error: string }>(
        "pi-rpc-error",
        (event) => {
          if (event.payload.session_id === sessionId) {
            setConsultError(event.payload.error);
            setConsultBusy(false);
          }
        },
      );
      ...
      await invoke("send_pi_command", {
        sessionId,
        jsonLine: JSON.stringify({ type: "prompt", message: system }),
      });
    } catch (e) {
      if (generation === consultantGeneration.current) {
  ...
  async function sendConsultMessage() {
    ...
    try {
      await invoke("send_pi_command", {
        sessionId: consultantId.current,
        jsonLine: JSON.stringify({ type: "prompt", message }),
      });
    } catch (e) {
      setConsultError(String(e));
      setConsultBusy(false);
    ...
    <>
      <div className="pipeline-settings">
        <section className="pipeline-target">
          <div>
            <small>APPLIES TO PROJECT</small>
            <strong>{projectName ?? projectPath.split("/").pop()}</strong>
            <span>{projectPath}</span>
          </div>
          <div className="pipeline-summary">
            <strong>{enabledCount}</strong>
            <span>of {config.steps.length} steps enabled</span>
          </div>
        </section>

        <div className="settings-notice">
          <strong>LOCAL SHELL ACCESS</strong>
          <span>
            Commands run from this registered project root. Review every command
            before saving.
          </span>
        </div>
        ...
        <section className="pipeline-preset-section">
          <div>
            <label htmlFor="pipeline-preset">STARTING PRESET</label>
            <span>
              Choose a template, then customize any step. Editing switches the
              preset to Custom.
            </span>
          </div>
          <div className="pipeline-preset-actions">
            <button
              className="pipeline-consult-button"
              onClick={() => {
                setConsultOpen(true);
                void startConsultant();
              }}
            >
              ✦ CONSULT AI
            </button>
            <select
              ...
              onChange={(event) =>
                void selectPreset(
                  event.target.value as PipelineConfig["preset"],
                )
              }
            >
              {PRESETS.map((preset) => (
        ...
        <div className="pipeline-step-header" aria-hidden="true">
          <span>ON</span>
          <span>STEP</span>
          <span>COMMAND</span>
        ...
        <div className="pipeline-step-list">
          {config.steps.map((step, index) => (
            <article
              className={`pipeline-step-editor ${step.enabled ? "" : "disabled"}`}
              key={step.id}
            >
              <label className="pipeline-step-toggle">
                <input
                  type="checkbox"
              ...
              <label>
                <span className="mobile-field-label">STEP</span>
                <input
                  value={step.name}
                  onChange={(event) =>
              ...
              <label>
                <span className="mobile-field-label">MODE</span>
                <select
                  className="themed-select"
                  aria-label={`${step.name} mode`}
                  value={step.mode ?? "shell"}
                  onChange={(event) =>
                    update(index, {
                      mode: event.target.value as PipelineStep["mode"],
                    })
                  }
                >
                  <option value="shell">Shell</option>
                  <option value="ai_commit">AI commit</option>
                  <option value="confirm">Confirm</option>
                </select>
              </label>
              <label>
                <span className="mobile-field-label">COMMAND</span>
                <input
                  className="pipeline-command-input"
                  value={step.command}
                  onChange={(event) =>
                    update(index, { command: event.target.value })
                  }
                  aria-label={`${step.name} command`}
                  placeholder={
                    (step.mode ?? "shell") === "ai_commit"
                      ? "Handled safely by app"
                      : "Shell command"
                  }
                  disabled={(step.mode ?? "shell") === "ai_commit"}
                  spellCheck={false}
                />
              </label>
              {(step.mode ?? "shell") !== "shell" && (
                <label>
                  <span className="mobile-field-label">PROMPT / OPTIONS</span>
                  <input
                    value={step.prompt ?? ""}
                    onChange={(event) =>
                  ...
                  {step.mode === "confirm" && (
                    <input
                      ...
                      placeholder="patch, minor, major"
                    />
                  )}
                </label>
              )}
              <label>
                <span className="mobile-field-label">ON FAILURE</span>
                <select
                  aria-label={`${step.name} failure policy`}
                  className="themed-select"
                  value={step.failure_policy}
                  title={
                    POLICIES.find(
                      (policy) => policy.value === step.failure_policy,
                    )?.detail
                  }
                  onChange={(event) =>
                    update(index, {
                      failure_policy: event.target
                        .value as PipelineStep["failure_policy"],
                    })
                  }
                >
                  {POLICIES.map((policy) => (
              ...
              <label>
                <span className="mobile-field-label">TRIES</span>
                <input
                  type="number"
                  min="1"
                  max="10"
                  ...
                  onChange={(event) =>
                    update(index, { max_attempts: Number(event.target.value) })
                  }
                  aria-label={`${step.name} max attempts`}
                />
              </label>
        ...
        <details className="pipeline-policy-guide">
          <summary>FAILURE POLICY HELP</summary>
          <div>
            {POLICIES.map((policy) => (
              <p key={policy.value}>
                <b>{policy.label}</b>
        ...
        {invalid && (
          <div className="settings-error" role="alert">
            Every step needs a name, command, and 1–10 attempts.
          </div>
        )}
        {error && (
          <div className="settings-error" role="alert">
      ...
      {consultOpen && (
        <aside
          ref={consultantRef}
          className="pipeline-consult-drawer"
          role="dialog"
          aria-modal="true"
          aria-label="AI pipeline consultant"
          tabIndex={-1}
        >
          <header>
            <div>
              <small>PROJECT COPILOT</small>
              <strong>PIPELINE CONSULTANT</strong>
            </div>
            <button
              onClick={closeConsultant}
              aria-label="Close pipeline consultant"
            >
              ×
            </button>
          </header>
          <div className="pipeline-consult-copy">
            Describe what should happen before push, merge, deploy, or release.
            The AI asks questions, then returns a draft for review.
          </div>
          <div className="pipeline-consult-chat">
          ...
          <div
            className="pipeline-consult-status"
            role="status"
            aria-live="polite"
          >
            {consultBusy
          ...
          {draft && (
            <section className="pipeline-consult-draft">
              <header>
                <div>
                  <strong>DRAFT READY</strong>
                  <span>
                    {draft.steps.length} steps · commands are not run or saved
                    yet
                  </span>
                </div>
                <button
                  onClick={() => {
                    setConfig({ ...draft, preset: "Custom" });
                    setDraft(null);
                  }}
                >
                  APPLY TO FORM
          ...
          <footer>
            <textarea
              ...
              placeholder="Example: test and build first, ask before pushing to main…"
              aria-label="Pipeline consultation message"
            />
            <button
              onClick={() => void sendConsultMessage()}
              disabled={!consultInput.trim() || consultBusy}
            >
              SEND
            </button>
          </footer>
      ...
      <footer>
        <span>
          {dirty
            ? `UNSAVED · ${projectName ?? "PROJECT"}`
            : `SAVED · ${projectName ?? "PROJECT"}`}
        </span>
        <div>
          <button
            onClick={() => setConfig(JSON.parse(saved) as PipelineConfig)}
