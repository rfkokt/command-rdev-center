...
export default function ProjectList({
  ...
  >({});
  const [repositoryBases, setRepositoryBases] = useState<
    Record<string, string>
  >({});
  ...
  async function addProject() {
    ...
    if (!path) return;
    try {
      const discovered = await invoke<ProjectInfo[]>("discover_projects", {
        path,
      });
        ...
        setProjects((prev) => [
          ...prev.filter(
            (item) =>
              ...
              !item.path.startsWith(`${workspace.path}/`),
          ),
          workspace,
        ]);
        setErr(null);
      ...
      const nextBranches = await invoke<string[]>("list_project_branches", {
        path,
      });
  ...
  async function editBaseBranch(project: ProjectInfo) {
    try {
      const nextBranches = project.is_git
        ? await invoke<string[]>("list_project_branches", {
            path: project.path,
          })
        : [];
      setBranches(nextBranches);
      ...
      const repositoryEntries = await Promise.all(
        (project.repositories ?? []).map(async (repository) => {
          const options = await invoke<string[]>("list_project_branches", {
            path: repository.path,
          });
          return [
            repository.path,
            options,
            repository.base_branch ?? options[0] ?? "",
          ] as const;
        }),
      );
      setRepositoryBranches(
        Object.fromEntries(
          repositoryEntries.map(([path, options]) => [path, options]),
        ),
      );
      setRepositoryBases(
        Object.fromEntries(
  ...
  async function saveBaseBranch() {
    ...
    try {
      const project = await invoke<ProjectInfo>("update_project_base_branch", {
          ...
          item.path === project.path
            ...
            : {
                ...
                repositories: item.repositories?.map((repository) =>
                  repository.path === project.path ? project : repository,
                ),
              },
        ),
      );
  ...
  async function fetchAllRepositoryBranches() {
    ...
    try {
      const fetched = await Promise.all(
        projectToEdit.repositories.map(
          async (repository) =>
            [
              repository.path,
              await invoke<string[]>("fetch_project_branches", {
                path: repository.path,
              }),
            ] as const,
        ),
  ...
  async function saveRepositoryBaseBranch(repository: ProjectInfo) {
    ...
    if (!selected) return;
    try {
      const updated = await invoke<ProjectInfo>("update_project_base_branch", {
        path: repository.path,
        baseBranch: selected,
      });
      setProjects((prev) =>
  ...
  async function loadSheets() {
    ...
    try {
      ...
      setTaskSource((source) => ({
        ...source,
        sheets: source.sheets.filter((sheet) => sheets.includes(sheet)),
      }));
      setAvailablePics([]);
      setErr(null);
    } catch (e) {
  ...
  async function loadPics() {
    if (!taskSource.url.trim()) return setErr("Google Sheets URL is required");
    if (taskSource.sheets.length === 0)
      return setErr("Select at least one worksheet");
    setLoadingPics(true);
    try {
      const pics = await invoke<string[]>("list_google_sheet_pics", {
        url: taskSource.url,
        sheets: taskSource.sheets,
      });
      setAvailablePics(pics);
      setTaskSource((source) => ({
        ...source,
        pics: source.pics.filter((pic) => pics.includes(pic)),
      }));
      setErr(null);
    } catch (e) {
      setErr(String(e));
  ...
  async function saveTaskSource() {
    ...
    try {
      setTaskSource(
        await invoke<TaskSource>("save_project_task_source", {
          path: projectToEdit.path,
          source: taskSource,
        }),
      );
      window.dispatchEvent(
        new CustomEvent("task-source-saved", {
          detail: { project: projectToEdit.name },
        }),
      );
      onToast(
        "Task source saved. Tasks refreshed from the selected worksheets.",
      );
      setErr(null);
    } catch (e) {
  ...
  async function removeProject() {
    ...
    try {
      ...
      setProjects((prev) =>
        prev.filter((item) => item.path !== projectToDelete.path),
      );
      setProjectToDelete(null);
      setErr(null);
    } catch (e) {
  ...
  return (
    <div className="projects-panel">
        ...
        createPortal(
          <div className="project-branch-backdrop" role="presentation">
            <div
              ...
              className="project-branch-picker"
              role="dialog"
              aria-modal="true"
              aria-labelledby="base-branch-title"
              tabIndex={-1}
            >
              ...
              <div>
                <button onClick={() => registerProject()}>REGISTER</button>
        ...
        createPortal(
          <div className="project-branch-backdrop" role="presentation">
            <div
              ...
              className="project-branch-picker project-settings-dialog"
              role="dialog"
              aria-modal="true"
              aria-labelledby="edit-base-branch-title"
              tabIndex={-1}
            >
              ...
              {projectToEdit.is_git && (
                <section>
                  ...
                  />
                  <p>
                    New chats use this branch. Existing chat baselines stay
                    unchanged.
                  </p>
                  <button
                    className="project-save-branch"
                    onClick={saveBaseBranch}
                  >
                    SAVE BRANCH
                  </button>
                </section>
              ...
              {!projectToEdit.is_git &&
                !!projectToEdit.repositories?.length && (
                  <section className="workspace-repository-settings">
                    <h3>REPOSITORIES</h3>
                    <p>
                      Each repository uses its own base branch for new chat
                      worktrees and diff baselines.
                    </p>
                    <button
                      type="button"
                      className="project-load-pics"
                      disabled={fetchingBranches}
                      onClick={() => void fetchAllRepositoryBranches()}
                    >
                      {fetchingBranches
                        ? "FETCHING BRANCHES…"
                        : "FETCH ALL BRANCHES"}
                    </button>
                    {projectToEdit.repositories.map((repository) => (
                      <div
                        className="workspace-repository-setting"
                        key={repository.path}
                      >
                        <strong>{repository.name}</strong>
                        <ListPicker
                          label="BASE BRANCH"
                          value={repositoryBases[repository.path] ?? ""}
                          options={repositoryBranches[repository.path] ?? []}
                          includeAll={false}
                          onChange={(value) =>
                            setRepositoryBases((current) => ({
                              ...current,
                        ...
                        <button
                          className="project-save-branch"
                          onClick={() =>
                            void saveRepositoryBaseBranch(repository)
                          }
                        >
                          SAVE BRANCH
                        </button>
      ...
      {projects.length === 0 && !err && (
        <div className="project-empty">INDEX EMPTY</div>
      )}
      {projectToDelete &&
        createPortal(
          <div className="project-branch-backdrop" role="presentation">
            <div
              ref={removeDialogRef}
              className="project-branch-picker project-remove-dialog"
              role="alertdialog"
              aria-modal="true"
            ...
            >
              ...
              <strong id="remove-project-title">
                Remove {projectToDelete.name}?
              </strong>
              <span>{projectToDelete.path}</span>
              <p>
                <b>Repository files are safe.</b> This only removes the project
                registration and its base-branch setting.
              </p>
              <div className="project-dialog-actions">
                <button
      ...
      {projects.map((project) => {
        const projectTabs = tabs.filter(
          (tab) => tab.project.path === project.path,
        );
