...
export default function ProjectFilesSidebar({
  ...
  const previewRef = useModalFocus<HTMLDivElement>(
    closePreview,
          ...
          (prev) =>
            new Set([
              ...
              ...[...prev].filter(
                (path) =>
                  path &&
                  list.some((file) => file.relative.startsWith(`${path}/`)),
              ),
            ]),
        );
      })
        ...
        if (isPreviewableImage(file.name)) {
          ...
          setPreview({
            ...
            imageSrc: URL.createObjectURL(
              new Blob([new Uint8Array(bytes)], { type: imageMime(file.name) }),
            ),
          });
  ...
  const tree = useMemo(
    () => (filteredFiles ? null : buildTree(files)),
    [files, filteredFiles],
  );
    ...
    >
      ...
      {recentPaths.length > 0 && !q && (
        <details className="pfs-recent">
          <summary>
            <span>Recent</span>
            <small>{Math.min(recentPaths.length, 5)}</small>
          </summary>
          {recentPaths.slice(0, 5).map((p) => (
            ...
            >
              ...
              <small>
                {p.includes("/") ? p.slice(0, p.lastIndexOf("/")) : "workspace"}
              </small>
            </button>
          ))}
        </details>
          ...
          <ul className="pfs-list pfs-flat" role="list">
            {filteredFiles.map((f) => (
              <li key={f.path} role="listitem">
                ...
                >
                  <span
                    className={`pfs-file-icon file-${fileKind(f.name)}`}
                    aria-hidden
                  >
                    {iconFor(f.name)}
                  </span>
                  <span
                    className="pfs-file-name"
      ...
      {(previewLoading || preview) &&
        createPortal(
          <div className="file-preview-backdrop">
            <div
              ref={previewRef}
              className="diff-panel file-preview-dialog"
              role="dialog"
              aria-modal="true"
              aria-label={
                preview ? `Preview ${preview.name}` : "Loading file preview"
              }
              tabIndex={-1}
              style={{
            ...
            >
              ...
              {previewLoading ? (
                <div style={{ margin: "auto", color: "var(--colors-muted)" }}>
                  LOADING FILE…
                </div>
              ) : (
                preview &&
                      ...
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          const possible = [
                            ...
                            `${resolved}/index.tsx`,
                            target.dataset.path,
                            target.dataset.path.slice(1),
                          ];
                          const found = files.find((file) =>
                            possible.includes(file.relative),
                          );
                          if (found) void openFile(found);
                        }}
                      >
                        {lines.map((line, index) => (
                          <div className="pfs-code-line" key={index}>
                            <span className="pfs-code-line-number" aria-hidden>
                              {index + 1}
                            </span>
                            <code
              ...
              <footer className="file-preview-footer">
                <span>
                  {preview
                    ? preview.imageSrc
                      ? "Image preview"
                      : `${splitLines(preview.content).length.toLocaleString()} lines · ${preview.content.length.toLocaleString()} chars`
                    : ""}{" "}
                  · Edit via chat to make changes.
                </span>
              </footer>
            </div>
          </div>,
...
function TreeFolder({
      ...
      {sortedFiles.map((f) => (
        <li key={f.path}>
          ...
          >
            ...
            <small className="pfs-file-meta">
              {f.chars ? shortChars(f.chars) : "bin"}
            </small>
          </button>
        </li>
      ))}
  ...
  return (
    <li>
      <button
        className={`pfs-folder${depth === 1 ? " repository-root" : ""}`}
        style={{ paddingLeft: 8 + (depth - 1) * 12 }}
        onClick={() => toggleFolder(folder.rel)}
        aria-expanded={isExpanded}
      >
        <span className="pfs-folder-chevron" aria-hidden>
          {isExpanded ? <ChevronDownIcon /> : <ChevronRightIcon />}
        </span>
        <span className="pfs-folder-icon" aria-hidden>
...
function highlightCode(code: string) {
  let html = code
    ...
    .replace(/>/g, "&gt;");
  return html.replace(
    /(\/\/.*|\/\*[\s\S]*?\*\/)|(["'])(.*?)\2|\b(import|export|from|const|let|var|function|async|await|class|interface|type|if|else|return|for|while|switch|case|default|break|continue|try|catch|finally|true|false|null|undefined|string|number|boolean)\b/g,
    (match, comment, quote, innerString, keyword) => {
      if (comment) return `<span class="syntax-comment">${comment}</span>`;
      if (quote) {
        const isPath =
          innerString.startsWith(".") ||
          innerString.startsWith("/") ||
          innerString.startsWith("@");
        const dataAttr = isPath
          ? ` class="clickable-import" data-path="${innerString}" style="text-decoration: underline;" title="Cmd+Click to go"`
          : "";
