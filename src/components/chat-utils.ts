...
export function shouldShowChanges(
...
export function projectTaskIntent(input: string): TaskIntent | null {
  ...
  const number = text.match(
    /\b(?:task|tugas|point|poin)(?:\s+(?:nomor|no))?\s+(\d+)\b/,
  )?.[1];
  if (
    number &&
    /\b(lihat|detail|cek|check|show|buka|apa|what|isi)\b/.test(text)
  )
    return { kind: "detail", taskNo: number };
  if (/\b(ada|list|daftar|available|tersedia|punya|what|apa)\b/.test(text))
    return { kind: "list" };
  return null;
}
...
export function ensureAssistantTurn(
...
export function shouldToastPiStderr(line: string) {
  ...
  return (
    Boolean(text) &&
    !text.startsWith("[crc-isolation ") &&
    !text.startsWith("Ponytail loaded:")
  );
}
...
export function settleWithError(
  messages: ChatMessage[],
  error: string,
): ChatMessage[] {
  ...
  return {
    ...(kind === "finished"
      ? {
          ...
          body: preview
            ? `${projectName}: ${preview}`
            : `${projectName} siap ditinjau.`,
          sound: "Glass",
        }
      : {
          title: "Agent perlu jawaban",
          body: `${projectName} menunggu respons.`,
          sound: "Ping",
        }),
