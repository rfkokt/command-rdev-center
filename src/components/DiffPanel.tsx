...
export default function DiffPanel({
  ...
  useEffect(() => {
    if (open && diff === undefined) refresh();
  }, [open, diff, refresh]);

  return (
    <div className="diff-backdrop" onMouseDown={onClose}>
      <aside
        ref={dialogRef}
      ...
      >
        <header>
          ...
          <span>
            {editingFile
              ? `EDITING ${editingFile}`
              : currentDiff
                ? `${currentDiff.files.length} FILES`
                : "VIEW DIFF"}
          </span>
          <button
            onClick={refresh}
            disabled={isLoading}
            aria-label={isLoading ? "Loading changes" : "Refresh changes"}
            title={isLoading ? "Loading changes" : "Refresh changes"}
          >
            {isLoading ? "…" : "↻"}
          </button>
          <button
        ...
        <div className="diff-files">
          {!isLoading && currentDiff?.files.length === 0 && (
            <p>WORKTREE CLEAN</p>
          )}
          {currentDiff?.files.map((file) => (
            <details
              key={`${file.repository ?? ""}-${file.path}-${editingFile === file.path}`}
              open={editingFile === file.path || undefined}
            >
              <summary>
              ...
              {file.patch ? (
                ...
                <p>Binary or untracked file — no textual diff.</p>
              )}
            </details>
          ))}
        </div>
        <button
          ...
          onClick={onHandoff}
          disabled={!currentDiff?.files.length}
        >
          SHIP CHANGES
        </button>
      </aside>
