...
describe("MarkdownMessage", () => {
  ...
  test("adds a copy control to fenced code blocks", () => {
    const { getByRole } = render(createElement(MarkdownMessage, null, "```ts\nconst answer = 42;\n```"));
    expect(getByRole("button", { name: "COPY" })).toBeTruthy();
  });

  test("defers Markdown parsing for oversized responses", () => {
    const { getByRole, container } = render(createElement(MarkdownMessage, null, "x".repeat(100_001)));
    expect(getByRole("button", { name: /Large response/ })).toBeTruthy();
    expect(container.querySelector(".markdown-body")).toBeNull();
  });

  test("renders KaTeX math and sanitizes unsafe HTML", () => {
    const { container } = render(createElement(MarkdownMessage, null, "$x^2$ <script>alert(1)</script><b>safe</b>"));
  ...
  test("offers Mermaid diagram preview", () => {
    const { getByRole } = render(createElement(MarkdownMessage, null, "```mermaid\ngraph TD; A-->B;\n```"));
