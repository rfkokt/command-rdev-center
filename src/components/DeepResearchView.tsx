...
function reportPresentation(run: ResearchRun) {
...
export default function DeepResearchView({ initialRunId, embedded = false, originChatId, originSessionId, onCompleted, onCreateDocumentary }: { initialRunId?: string | null; embedded?: boolean; originChatId?: string; originSessionId?: string; onCompleted?: (run: ResearchRun) => void; onCreateDocumentary?: (run: ResearchRun) => void }) {
  ...
  async function remove(run: ResearchRun) { if (!window.confirm(`Delete research "${run.query}"? This cannot be undone.`)) return; setBusy(true); try { await invoke("delete_deep_research", { runId: run.id }); setSelected(null); await load(); } catch (x) { setError(String(x)); } finally { setBusy(false); } }
