...
export default function ListPicker({ label, value, options, allLabel = "All", includeAll = true, onChange }: Props) {
  ...
  return <label className="list-picker-field" ref={rootRef}>{label}
    <button type="button" className="list-picker-trigger" onClick={() => setOpen((current) => !current)} aria-haspopup="listbox" aria-expanded={open}>{value || allLabel}<span>{open ? "⌃" : "⌄"}</span></button>
