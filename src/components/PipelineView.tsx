...
export default function PipelineView() {
  ...
  useEffect(() => {
    const load = () => invoke<PipelineData>("get_pipeline_data").then((pipeline) => {
      setData(pipeline);
      setError(null);
  ...
  return <section className="pipeline-view">
    ...
    {error ? <p className="kanban-error">{error}</p> : !runs.length ? <div className="pipeline-empty">No pipeline runs yet.</div> : <div className="pipeline-scroll">
      {projectTypes.map((projectType) => {
        ...
        const completed = data.runs.filter((run) => run.project_type === projectType);
        const averages = Object.fromEntries(columns.map((name) => {
          const times = completed.flatMap((run) => run.stages.filter((stage) => stage.name === name && stage.ms).map((stage) => stage.ms ?? 0));
