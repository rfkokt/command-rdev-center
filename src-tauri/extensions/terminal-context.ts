...
const destructive = (command: string) =>
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Execute a non-destructive local shell, SSH, or device command and return its cleaned stdout/stderr directly. Destructive commands are held for host approval. Use this instead of write/read polling for agent work.",
    parameters: Type.Object({
      ...
      timeoutSeconds: Type.Optional(
        Type.Number({
          minimum: 1,
          maximum: 120,
          description: "Timeout; defaults to 30 seconds",
        }),
      ),
    }),
    async execute(_id, input) {
      if (destructive(input.command)) {
        return {
          ...
          details: { destructive: true, command: input.command },
        };
      }
      const timeout =
        Math.min(120, Math.max(1, input.timeoutSeconds || 30)) * 1000;
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify({
                status: urls.length ? "awaiting_user_action" : "completed",
                output,
                exitCode: 0,
                urls,
              }),
          ...
          details: { output, exitCode: 0, urls },
        };
      } catch (error) {
        ...
        return {
          ...
          details: { output, exitCode: failure.code, urls },
          isError: false,
        };
      }
    },
  ...
  pi.registerTool({
    ...
    description:
      "List active in-app terminal panes belonging to this chat only.",
    parameters: Type.Object({}),
    async execute() {
      const files = await panes();
      const text = files.length
        ? files.map((file) => `- Pane ${paneId(file)}`).join("\n")
        : "No active terminal panes for this chat.";
      return {
        content: [{ type: "text", text }],
        details: { panes: files.map(paneId) },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description:
      "Read recent output from one active in-app terminal pane in this chat. Use this to inspect build failures, stack traces, warnings, and process output. ANSI formatting is removed.",
    parameters: Type.Object({
      pane: Type.Optional(
        Type.String({
          description: "Pane number from list_chat_terminals; defaults to 0",
        }),
      ),
      maxChars: Type.Optional(
        Type.Number({
          minimum: 1000,
          maximum: 50000,
          description: "Maximum recent characters; defaults to 12000",
        }),
      ),
    }),
    async execute(_id, input) {
      ...
      if (!(await panes()).includes(file))
        throw new Error(`Terminal pane ${pane} is not active in this chat`);
      ...
      const handoff = urls.length
        ...
        : "";
      return {
        content: [{ type: "text", text: visible + handoff }],
        details: { pane, truncated: output.length > maxChars, urls },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description:
      "Send exact input to an active in-app terminal pane. Non-destructive commands run automatically; destructive commands are held for user approval by the host. Include a newline when the command should execute.",
    parameters: Type.Object({
      pane: Type.Optional(
        Type.String({ description: "Pane number; defaults to 0" }),
      ),
      data: Type.String({
        minLength: 1,
        maxLength: 10000,
        description:
          "Exact terminal input, including \\n when Enter should be pressed",
      }),
    }),
    async execute(_id, input) {
      return {
        content: [
          {
            type: "text",
            text: "Terminal input submitted to the host. Safe commands run immediately; read this pane again for output and finish the user's request in this same turn. Do not claim approval is pending unless the command is destructive.",
          },
        ],
        details: { pane: input.pane || "0", bytes: input.data.length },
      };
    },
  });
}
