...
function taskMarkdown(task: Task, allTasks: Task[]) {
  ...
  const extra = Object.entries(task)
    .filter(
      ([key, value]) =>
  ...
  const referenceNumbers = [
    ...text.matchAll(/(?:poin|point)(?: nomor)?\s+(\d+)/g),
  ].map((match) => match[1]);
  const references = embeddedReferences.length
    ? embeddedReferences
    : allTasks.filter((candidate) =>
        referenceNumbers.includes(String(candidate.no)),
      );
  return [
    `# Task #${task.no ?? "—"}`,
    "",
    ...
    ...references.flatMap((reference) => [
      "",
      `## Referenced task #${reference.no ?? "—"}`,
      "",
      String(reference.deskripsi || "Untitled task"),
      reference.notes ? `\n### Notes\n\n${reference.notes}` : "",
    ]),
  ]
    .filter(Boolean)
...
function kanbanMarkdown(items: Task[]) {
  ...
  const sections = [...groups.entries()].map(([status, group]) =>
    [
      ...
      ...group.map(
        (task) =>
          `| ${oneLine(task.no, 20) || "—"} | ${oneLine(task.deskripsi) || "Untitled task"} | ${oneLine(task.priority, 20) || "—"} | ${oneLine(task.pic, 30) || "—"} |`,
      ),
    ].join("\n"),
  );
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Show an interactive skill recommendation card after deciding that one or more installed global skills are relevant to the user's request. Do not call merely because keywords overlap; use your judgment. Use exact installed skill names and recommend at most three.",
    promptGuidelines: [
      "Call only after receiving the user's message and deciding a skill would materially help.",
      "Do not recommend skills for requests you can answer directly without procedural guidance.",
    ],
    parameters: Type.Object({
      skills: Type.Array(
        Type.String({ description: "Exact installed skill name" }),
        { minItems: 1, maxItems: 3 },
      ),
      reason: Type.String({
        description: "Concise explanation of why these skills fit",
      }),
    }),
    async execute(_id, input) {
      return {
        content: [
          { type: "text", text: "Skill recommendations shown to the user." },
        ],
        details: input,
      };
    },
  ...
  pi.registerTool({
    ...
    description:
      "After the user explicitly agrees, draft a backlog task for a verified application error so it can be prevented later. The host shows the draft for final approval before saving it.",
    promptGuidelines: [
      "Only call after identifying a real application error and the user explicitly agrees to report it. Include a concrete root cause and prevention.",
      "Do not use this for ordinary command failures that were fixed during the current task unless the user asks to report them.",
    ],
    parameters: Type.Object({
      title: Type.String({ description: "Short error summary" }),
      rootCause: Type.String({ description: "Verified technical root cause" }),
      prevention: Type.String({
        description: "Concrete change or check preventing recurrence",
      }),
    }),
    async execute(_id, input) {
      return {
        content: [
          {
            type: "text",
            text: "Error report draft submitted for final review.",
          },
        ],
        details: input,
      };
  ...
  pi.registerTool({
    ...
    async execute(_id, input) {
      ...
      const result = (await tasks()).filter((task) => {
        ...
        return (
          (!input.pic ||
            task.pic?.trim().toLowerCase() ===
              input.pic.trim().toLowerCase()) &&
          (requestedStatus ? status === requestedStatus : ready.has(status))
        );
      });
      return {
  ...
  pi.registerTool({
    ...
    description:
      "Create or update the current chat's Kanban task. Call with In Progress only when you decide the user requested actionable work worth tracking; do not call for questions or discussion. Call with Done only when the user explicitly accepts the completed work.",
    promptSnippet: "Track actionable work in the local Kanban",
    promptGuidelines: [
      "Use track_kanban_task when you decide a user request is actionable work worth tracking; do not ask permission and do not use language/keyword heuristics.",
    ],
    parameters: Type.Object({
    ...
    async execute(_id, input) {
      return {
        content: [
          { type: "text", text: `Kanban task marked ${input.status}.` },
        ],
        details: input,
      };
    },
