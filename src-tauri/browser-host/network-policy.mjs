...
export function validateNavigationUrl(value, approvedOrigins) {
  ...
  if (!approvedOrigins.has(url.origin))
    throw new Error("origin_approval_required");
  return url;
}
...
export function requestPolicy({
  ...
  if (LOCAL_HOSTS.has(document.hostname) && url.protocol === "https:")
    return { disposition: "allowed", reason: "local_app_backend" };
  if (!SAFE_METHODS.has(method.toUpperCase()))
    throw new Error("cross_origin_mutation_blocked");
  if (document.protocol !== url.protocol)
    throw new Error("cross_origin_class_transition_blocked");
...
export async function validateDestinationUrl(
  ...
  return validateResolvedUrl(url, lookup, allowPrivate);
}

export async function validateRequestNetwork(value, documentOrigin, lookup) {
  const target = await validateResolvedUrl(validateUrl(value), lookup, true);
  if (!documentOrigin) return target;
  const document = await validateResolvedUrl(
    validateUrl(documentOrigin),
    lookup,
    true,
  );
  const localAppBackend =
    document.networkClass === "loopback" &&
    validateUrl(value).protocol === "https:" &&
    ["public", "private"].includes(target.networkClass);
  if (target.networkClass !== document.networkClass && !localAppBackend)
    throw new Error("cross_origin_class_transition_blocked");
  return target;
}

async function validateResolvedUrl(url, lookup, allowPrivate = false) {
  const { networkClass } = await resolveDestination(url.hostname, lookup);
  const allowed =
    networkClass === "loopback" ||
    (networkClass === "public" && url.protocol === "https:") ||
    (allowPrivate && networkClass === "private" && url.protocol === "https:");
