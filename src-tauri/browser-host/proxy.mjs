...
export function classifyAddress(address) {
  ...
  if (mapped)
    return classifyAddress(mapped[1]) === "public"
      ? "public"
      : "mapped-private";
...
export async function resolveDestination(hostname, lookup = dns.lookup) {
  ...
  const classes = new Set(
    records.map(({ address }) => classifyAddress(address)),
  );
...
function authorize(headers, token) {
  ...
  return (
    value === `Basic ${Buffer.from(`Bearer:${token}`).toString("base64")}` ||
    value === `Bearer ${token}`
  );
}
...
export function createEnforcingProxy({
  token,
  allowLoopback = true,
  allowPublicHttps = false,
  allowPrivateHttps = false,
  lookup = dns.lookup,
  connect = net.connect,
} = {}) {
  ...
  const validate = async (hostname, scheme, port) => {
    const { records, networkClass } = await resolveDestination(
      hostname,
      lookup,
    );
    const allowed =
      (networkClass === "loopback" && allowLoopback) ||
      (networkClass === "public" &&
        allowPublicHttps &&
        scheme === "https:" &&
        port === 443) ||
  ...
  const deny = (socket, status, code) => {
    ...
    if (typeof socket.writeHead === "function") {
      socket.writeHead(
        ...
        statusCode === 407
          ? {
              "proxy-authenticate": 'Basic realm="Kern Browser Host"',
              connection: "close",
            }
          : { connection: "close" },
      );
  ...
  const server = http.createServer(async (request, response) => {
    if (!authorize(request.headers, token))
      return deny(
        response,
    ...
    try {
      ...
      if (target.username || target.password || target.protocol !== "http:")
        throw new Error("unsupported_or_credential_url");
      const destination = await validate(
        target.hostname,
        target.protocol,
      ...
      upstream.on("error", () =>
        deny(response, "502 Bad Gateway", "upstream_failed"),
      );
      request.pipe(upstream);
      events.push({
        disposition: "allowed",
        networkClass: destination.networkClass,
        scheme: "http:",
      });
    } catch (error) {
      deny(response, "403 Forbidden", error.message);
    }
  });
  server.on("connect", async (request, client, head) => {
    ...
    try {
      ...
      deny(client, "403 Forbidden", error.message);
    }
  });
  return {
    events,
